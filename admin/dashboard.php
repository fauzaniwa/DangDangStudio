...
$pm_names = [];
while($p = mysqli_fetch_assoc($q_pm)) { $pm_names[] = $p['name']; }
...
$fin_labels = []; $fin_in = []; $fin_out = [];
while($f = mysqli_fetch_assoc($q_finance)) { $fin_labels[] = $f['bulan']; $fin_in[] = $f['masuk']; $fin_out[] = $f['keluar']; }

$q_industry = mysqli_query($conn, "SELECT category, COUNT(*) as jml FROM clients GROUP BY category");
$ind_labels = []; $ind_counts = [];
while($ind = mysqli_fetch_assoc($q_industry)) { $ind_labels[] = $ind['category'] ?? 'Other'; $ind_counts[] = $ind['jml']; }
...
<body class="text-slate-900">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8 scroll-hide">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    <div class="glass-card p-8 rounded-[40px] border-b-4 border-brandTeal shadow-xl relative overflow-hidden group">
                        ...
                        <div class="absolute -right-6 -bottom-6 text-brandTeal/5"><svg class="w-32 h-32" fill="currentColor" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
                    </div>
                    ...
                    <div class="glass-card p-8 rounded-[40px] border-b-4 border-rose-500 shadow-xl relative overflow-hidden group">
                        ...
                        <div class="absolute -right-6 -bottom-6 text-rose-500/5"><svg class="w-32 h-32" fill="currentColor" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg></div>
                    </div>
                    ...
                    <div class="bg-white rounded-[40px] p-8 shadow-sm border border-slate-100">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0"><h4 class="text-xs font-bold text-brandPrimary truncate"><?= $art['title'] ?></h4><p class="text-[10px] text-slate-400 italic"><?= date('d M Y', strtotime($art['created_at'])) ?></p></div>
                            </div>
                            <?php endwhile; ?>
                        </div>
                    </div>
                        ...
                        <div class="bg-brandTeal rounded-[40px] p-8 text-white shadow-xl relative overflow-hidden">
                            ...
                            <div class="mb-4 last:mb-0 p-3 bg-white/10 rounded-2xl border border-white/20">
                                <div class="flex gap-1 mb-1">
                                    <?php for($i=1;$i<=5;$i++): ?>
                                    <svg class="w-2.5 h-2.5 <?= ($i <= ($tes['stars'] ?? 5)) ? 'text-brandGold' : 'text-white/20' ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    <?php endfor; ?>
                                </div>
    ...
    <script>
        ...
        new Chart(document.getElementById('financialChart'), {
            ...
            data: {
                ...
                datasets: [
                    { label: 'In', data: <?= json_encode($fin_in) ?>, backgroundColor: '#14B8A6', borderRadius: 8 },
                    { label: 'Out', data: <?= json_encode($fin_out) ?>, backgroundColor: '#E2E8F0', borderRadius: 8 }
                ]
            },
        ...
        new Chart(document.getElementById('industryChart'), {
            ...
            data: {
                ...
                datasets: [{ data: <?= json_encode($ind_counts) ?>, backgroundColor: ['#1e1e1e', '#14B8A6', '#F59E0B', '#6366F1'], borderWidth: 0 }]
            },
