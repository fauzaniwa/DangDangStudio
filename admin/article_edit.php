...
// 1. Proteksi Halaman & Validasi ID
...
// 3. Ambil Data Galeri
$sql_gallery = "SELECT * FROM article_gallery WHERE article_id = '$id'";
...
<head>
    ...
    <title>Edit Article - DangDang Studio</title>
                        ...
                        <form action="process/process_edit_article.php" method="POST" enctype="multipart/form-data" class="space-y-8 pb-20">
                            ...
                            <div class="group relative">
                                ...
                                <div class="relative h-64 md:h-80 rounded-[32px] border-2 border-dashed border-gray-200 bg-white flex items-center justify-center overflow-hidden transition-all group-hover:border-brandGold/50 shadow-sm">
                                    <button type="button" id="btn-remove-cover" onclick="resetCover()" class="absolute top-4 right-4 z-40 bg-white/90 text-red-500 p-2 rounded-xl transition shadow-sm <?php echo empty($article['cover_image']) ? 'hidden' : ''; ?>">
                                    ...
                                    <div id="cover-placeholder" class="text-center z-20 <?php echo !empty($article['cover_image']) ? 'hidden' : ''; ?>">
                                        ...
                                        <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Upload New Cover</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 space-y-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="title" required value="<?php echo htmlspecialchars($article['title']); ?>" placeholder="Enter title..." class="w-full px-0 py-2 bg-transparent border-b-2 border-gray-100 focus:border-brandGold outline-none transition font-extrabold text-2xl md:text-3xl text-brandPrimary placeholder:text-gray-200">
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select name="status" required class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:border-brandGold outline-none transition font-bold text-sm">
                                            <option value="Draft" <?php echo ($article['status'] == 'Draft') ? 'selected' : ''; ?>>Save as Draft</option>
                                            <option value="Published" <?php echo ($article['status'] == 'Published') ? 'selected' : ''; ?>>Publish Now</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="content" rows="8" required placeholder="Start writing..." class="w-full px-6 py-5 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:border-brandGold outline-none transition text-gray-600 leading-relaxed"><?php echo htmlspecialchars($article['content']); ?></textarea>
                                </div>
                            </div>
                            ...
                            <div class="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 space-y-6">
                                <div class="flex items-center justify-between mb-2">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Manage your visual assets</p>
                                    </div>
                            ...
                            <div class="flex items-center justify-end gap-4 pb-10">
                                <a href="articles.php" class="px-8 py-4 font-bold text-gray-400 hover:text-brandAccent transition">Cancel</a>
                                <button type="submit" class="px-12 py-4 bg-brandGold text-white rounded-[20px] font-bold shadow-2xl shadow-brandGold/20 hover:scale-[1.05] transition-all">
                                    Update Article
                                </button>
                            </div>
                        </form>
            ...
            Array.from(input.files).forEach(file => {
                ...
                reader.onload = function(e) {
                    ...
                    wrapper.innerHTML = `
                        ...
                        <div class="absolute inset-0 bg-brandGold/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button type="button" onclick="this.parentElement.parentElement.remove()" class="bg-white p-2 rounded-full text-brandGold shadow-lg">
